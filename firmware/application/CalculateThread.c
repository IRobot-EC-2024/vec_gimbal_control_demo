...
#ifdef AUTOMATIC_INFANTRY

#include "usbd_cdc_if.h"
#include "USBTransporter.h"

uint32_t error_package_counter = 0;

int16_t LimitMax(int32_t input, int32_t max)
{
    if (input > max) {
        return max;
    }
...
struct SkiderGimbal {
    ...
    struct SkiderMotor {
        GimbalMotorMeasure_t    gimbal;
        ShootMotorMeasure_t     shooter;
    } motor;
...
{
    osDelay(500);
    usbcdc_message_transmit();
    while(1)
    {
        osDelay(1);
        usbcdc_message_transmit();
        GimbalEulerSystemMeasureUpdate(&SkiderGimbal.imu);
        GimbalMotorMeasureUpdate(&SkiderGimbal.motor.gimbal);
        ShootMotorMeasureUpdate(&SkiderGimbal.motor.shooter);
        
        DeviceOfflineMonitorUpdate(&Offline);
        
        
        // 通信断连保护
        if ((CDC_GetUpdateTimeout() > CDC_TRANSPORT_MAX_TIMEOUT)) {
            SkiderGimbal.output.yaw = 0;
            SkiderGimbal.output.pitch = 0;
            SkiderGimbal.output.rotor = 0;
            SkiderGimbal.output.ammol = 0;
            SkiderGimbal.output.ammor = 0;
            GimbalMotorControl( 0,
                                0, 
                                0, 
                                0, 
                                0
        ...
        SkiderControlFrame_SCM_t SkiderSCMFrame;
        CDC_GetSkiderSCMFrame(&SkiderSCMFrame);
        switch (CDC_GetUpdateSgn()) {
            // 通信错误保护
            case SKIDER_CONTROLMODE_SCM: {
                CDC_ClearUpdateSgn();
                
                if ((SkiderSCMFrame._SOF_ != 0x66)  ||  (SkiderSCMFrame._EOF_ != 0x88)) {
                    if (error_package_counter < CDC_TRANSPORT_MAX_ERROR) {
                        error_package_counter++;
                        break;
                    }
                    else {
                        error_package_counter++;
                ...
                error_package_counter = 0;
                
                SkiderGimbal.GimbalState = SkiderSCMFrame.GimbalState;
                SkiderGimbal.ShooterState = SkiderSCMFrame.ShooterState;
                
                SkiderGimbal.command.yaw = SkiderSCMFrame.YawCommand;
                SkiderGimbal.command.pitch = SkiderSCMFrame.PitchCommand;
                SkiderGimbal.command.rotor = SkiderSCMFrame.RotorCommand;
                SkiderGimbal.command.ammol = SkiderSCMFrame.AmmolCommand;
                SkiderGimbal.command.ammor = SkiderSCMFrame.AmmorCommand;
                
                SkiderGimbal.gain.yaw = SkiderSCMFrame.YawKp;
                SkiderGimbal.gain.pitch = SkiderSCMFrame.PitchKp;
                SkiderGimbal.gain.rotor = SkiderSCMFrame.RotorKp;
                SkiderGimbal.gain.ammol = SkiderSCMFrame.AmmolKp;
                SkiderGimbal.gain.ammor = SkiderSCMFrame.AmmorKp;
        ...
        if (SkiderGimbal.GimbalState == SKIDER_GIMBAL_ENABLE) {
            SkiderGimbal.output.yaw = LimitMax(((SkiderGimbal.command.yaw - SkiderGimbal.imu.YawSpeed) * SkiderGimbal.gain.yaw), 30000);
            SkiderGimbal.output.pitch = LimitMax(((SkiderGimbal.command.pitch - SkiderGimbal.imu.PitchSpeed) * SkiderGimbal.gain.pitch), 30000);
        ...
        if (SkiderGimbal.ShooterState == SKIDER_SHOOTER_ENABLE) {
            SkiderGimbal.output.rotor = LimitMax(((SkiderGimbal.command.rotor - SkiderGimbal.motor.shooter.RotorMotorSpeed) * SkiderGimbal.gain.rotor), 10000);
            SkiderGimbal.output.ammol = LimitMax(((SkiderGimbal.command.ammol - SkiderGimbal.motor.shooter.AmmoLeftMotorSpeed) * SkiderGimbal.gain.ammol), 16384);
            SkiderGimbal.output.ammor = LimitMax(((SkiderGimbal.command.ammor - SkiderGimbal.motor.shooter.AmmoRightMotorSpeed) * SkiderGimbal.gain.ammor), 16384);
        }
        else {
            SkiderGimbal.output.rotor = 0;
            SkiderGimbal.output.ammol = 0;
        ...
        GimbalMotorControl( SkiderGimbal.output.yaw * YAW_MOTOR_DIRECTION,
                            SkiderGimbal.output.pitch * PITCH_MOTOR_DIRECTION, 
                            SkiderGimbal.output.rotor, 
                            SkiderGimbal.output.ammol, 
                            SkiderGimbal.output.ammor
                            );
