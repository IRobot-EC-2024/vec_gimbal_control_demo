...
RemoteSensorNode::RemoteSensorNode(const rclcpp::NodeOptions & options)
{
    ...
    RCLCPP_INFO(remote_sensor_node_->get_logger(), "Init Timer 100Hz");
    timer_100Hz_ = remote_sensor_node_->create_wall_timer(10ms, std::bind(&RemoteSensorNode::loop_100Hz, this));
}

void RemoteSensorNode::loop_100Hz()
{
    remote_->process();
